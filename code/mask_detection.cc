...
int main(int argc, char** argv) {
    ...
    if (argc > 4 || argc < 3){
      std::cerr << "[ERROR] usage: " << argv[0]
                ...
                << "If you add a image path will recognize the image, otherwise recognize the videostream"
                << std::endl;
      exit(1);
    }

    std::string detect_model = argv[1];
    ...
    std::shared_ptr<PaddlePredictor> predictor2 = CreatePaddlePredictor<MobileConfig>(config2);
